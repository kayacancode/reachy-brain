...
class AudioStream:
    ...
    async def play_pcm(self, audio_data: np.ndarray, sample_rate: int = 48000) -> None:
        """Play PCM audio data through the robot's speaker.
        ...
        try:
            # Resample if needed
            robot_out_rate = media.get_output_audio_samplerate()
            if robot_out_rate != sample_rate and robot_out_rate > 0:
                new_length = int(len(audio_data) * robot_out_rate / sample_rate)
                audio_data = await asyncio.to_thread(resample, audio_data, new_length)
            ...
            logger.error(f"Error playing PCM: {e}")
        finally:
            self._is_speaking = False
