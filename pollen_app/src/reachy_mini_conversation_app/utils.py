...
def parse_args() -> Tuple[argparse.Namespace, list]:  # type: ignore
    ...
    # Auto-enable gradio when USE_CLAWDBOT is set (Clawdbot needs browser WebRTC for audio)
    use_clawdbot_env = os.getenv("USE_CLAWDBOT", "").lower() in ("1", "true", "yes")
    use_gradio_env = os.getenv("USE_GRADIO", "").lower() in ("1", "true", "yes")
    gradio_default = use_clawdbot_env or use_gradio_env
    ...
    parser.add_argument("--gradio", default=gradio_default, action="store_true", help="Open gradio interface")
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Use Clawdbot (Claude via OpenClaw) + ElevenLabs TTS + Honcho memory instead of OpenAI Realtime",
    )
